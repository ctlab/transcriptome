...
public class FillHoles {
    ...
    public static class Reduce extends MapReduceBase
         ...
         */
        private Pair<Filler,FillingReport> fillHole(int k, int phredTrim) {

            consensuses.clear();
            System.err.println("Filling using k = " + k + ", and phredTrim = " + phredTrim);
            WeightedDeBruijnGraph graph = new WeightedDeBruijnGraph(k, 1);
...
//            graph.addEdges(new DnaView(contigs[1].sequence, Math.max(minOffset1 - maxLength, 0), Math.min(contigs[1].sequence.length(), maxOffset1 + maxLength)));
            ...
            if (key.isOpen()) {
                ...
                for (int i = 0; i < nreads; ++i) {
                    ...
                    FillingTask.FillingResult res = rfTask.fillRead(leftHint, rightHint, true);
                    tries++;
                    /*
                    ...
                    */
                    ...
                    if (res.dnaq != null) {
                        ...
                        int extendingLength = res.dnaq.length() - leftOffset;
                        if (extendingLength > 0) {
                            tempView.set(res.dnaq, leftOffset, extendingLength);
                        } else {
                ...
                if (outputLength > 20) {
                    ...
                    outValue.weight = (int) (nreads * extendingConsensus.getLayersNumber() / tries/ rfTask.getProcessed());
                    outSequence.set(new DnaWritable(extendingConsensus.getDna(), 0, outputLength));
                    return ImmutablePair.make(outValue, rfTask.fillingReport);
                }


                return ImmutablePair.make(null, rfTask.fillingReport);
            }
...
//                System.err.println(DnaView.rcView(rightHint));
                ...
                if (res.dnaq != null) {
                    ...
                    int distance = res.dnaq.length() - leftOffset - rightOffset;
...
//                        System.err.println(res.dnaq);
                    ...
                    if (leftOffset >= 0 && rightOffset >= 0) {
                        if (distance > 0) {
                            tempView.set(res.dnaq, leftOffset, distance);
                        } else {
                            tempView.set(emptyDnaq, 0, 0);
                        }
...
//                    System.err.println("not found");
            ...
            if (rfTask.getOk() < 0.8 * (rfTask.getProcessed() - rfTask.getNotFound())) {
                System.err.println("too little ok");
                return ImmutablePair.make(null, rfTask.fillingReport);
            }
            if (rfTask.getAmbiguous() > 10) {
            ...
            for (Map.Entry<MutableInt, Consensus> entry: consensuses.entrySet()) {
                ...
                if (consensus.getLayersNumber() > totalRepaired * 0.8) {
                    ...
                    outValue.weight = (int) (consensus.getLayersNumber() * totalBridges / rfTask.getProcessed());
...
//                System.err.println("oriented: " + pread);
                if (isPreadForFilling(pread, false)) {
                    assert 
                        ...
                        -pread.first.dnaq.length() < pread.second.alignment.offset &&
...
//            System.err.println("oriented all");
            ...
            for (int i = 0; i < 2; ++i) {
                Pair<Filler, FillingReport> fillingResult = fillHole(curK, curPhredTrim);
