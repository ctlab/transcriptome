...
public class BucketsSorter extends Tool {
    ...
    private void bucketSortWithForking(final Executor executor, final SuffixArray sa, final int begin,
                                       final int len, final int charsSorted) {
        if (len > 10000) {
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    bucketSort(executor, sa, begin, len, charsSorted);
                }
            });
        } else {
            bucketSort(executor, sa, begin, len, charsSorted);
     ...
     * Sorts large arrays
     */
    private void bucketSort(final Executor executor, final SuffixArray sa, int begin, int len, int charsSorted) {
        int smallBucketCharsNumber = this.smallBucketCharsNumber;
        int smallBucketsNumber = this.smallBucketsNumber;
        if (charsSorted <= bucketCharsNumber) {
            smallBucketCharsNumber = 2;
            smallBucketsNumber = ALPHABET * ALPHABET;
        }
        ...
        for (int i = 0; i < smallBucketsNumber; i++) {
            if ((ei[i] - bi[i]) > 1) {
                ...
                if (!bucketCharsContain$) {
                    if ((ei[i] - bi[i]) < smallBucketsNumber) {
                        quickSort(sa, bi[i], ei[i] - bi[i], charsSorted);
                    } else {
                        bucketSortWithForking(executor, sa, bi[i], ei[i] - bi[i], charsSorted);
                    }
                }
            }
        }
